...
import (
	...
	avcodec_leo "github.com/leokinglong/goav/avcodec"
)
...
func doDecode(deCodecCtx *avcodec.Context, inPkt *avcodec.Packet, deFrame *avutil.Frame, enFrame *avutil.Frame,
	swsCtx *swscale.Context, EnCodecCtx *avcodec.Context, leoEnCodecCtx *avcodec_leo.Context, outFormat *avformat.Context, videoOutIndex int) {
	num_pts := 0
	//先解码
	deCodecCtx.AvcodecSendPacket(inPkt)
	for {
		ret := deCodecCtx.AvcodecReceiveFrame((*avcodec.Frame)(unsafe.Pointer(deFrame)))
		fmt.Println("ret:", ret)
		if ret == avutil.AvErrorEOF {
			doEncode(nil, EnCodecCtx, leoEnCodecCtx, outFormat, videoOutIndex)
			break
		} else if ret < 0 {
			break
		}
		//解码后，先进行格式转化再编码
		if swscale.SwsScale2(swsCtx, avutil.Data(deFrame), avutil.Linesize(deFrame), 0, deCodecCtx.Height(),
		...
		num_pts++
		doEncode(enFrame, EnCodecCtx, leoEnCodecCtx, outFormat, videoOutIndex)
	}
}

func doEncode(enFrame *avutil.Frame, enCodecCtx *avcodec.Context, leoEnCodecCtx *avcodec_leo.Context, outFormat *avformat.Context, videoOutIndex int) {
	// ret := enCodecCtx.AvcodecSendFrame((*avcodec.Frame)(unsafe.Pointer(enFrame)))

	ret := leoEnCodecCtx.AvcodecSendFrame((*avcodec_leo.Frame)(unsafe.Pointer(enFrame)))
	fmt.Println("ret:", ret)
	for {
		out_Pkt := avcodec.AvPacketAlloc()
		if leoEnCodecCtx.AvcodecReceivePacket((*avcodec_leo.Packet)(unsafe.Pointer(out_Pkt))) < 0 {
...
func img2Video2() {
	...
	leo_en_codec := avcodec_leo.AvcodecFindEncoder(avcodec_leo.CodecId(avcodec.AV_CODEC_ID_H264))
	if en_codec == nil {
		fmt.Println("Unsupported codec!")
		os.Exit(1)
	}
	...
	leo_en_ctx := leo_en_codec.AvcodecAllocContext3()
	// 设置h264编码参数
	inStream := in_fmt.Streams()[videoIndex]
	//配置编码器上下文的成员
	//设置画面宽度 高度 输出像素格式（pix_fmt)
	leo_en_ctx.SetEncodeParams(inStream.Codec().GetWidth(), inStream.Codec().GetHeight(), (avcodec_leo.PixelFormat)(inStream.Codec().GetPixelFormat()))
	...
	leo_en_ctx.SetTimebase(1, 25)
	en_ctx.SetTimebase(1, 25)
	// 设置全局编码信息
	// EnCodecCtx.Set= EnCodecCtx.Flags() | avcodec.AV_CODEC_FLAG_GLOBAL_HEADER
	//打开编码器
	if leo_en_ctx.AvcodecOpen2(leo_en_codec, nil) < 0 {
		fmt.Println("Could not open leo_en_codec")
		os.Exit(1)
	}
	...
	for frameNumber := 1; frameNumber < 30; frameNumber++ {
		...
		for { //while 循环
			...
			//先解码
			//doDecode
			doDecode(de_ctx, in_pkt, deFrame, enFrame, swsCtx, en_ctx, leo_en_ctx, ou_fmt, video_ou_index)
