...
// 	var imgMap map[string]([]([]byte))
// 	imgMap = make(map[string]([]([]byte))) //map[i]-----[]image
...
// 			//路径拼接
// 			srcPath := "./result/%d_%d_%.3f.jpg"                                                   //时间戳_id_置信度
// 			srcPathStr := fmt.Sprintf(srcPath, time.Now().Unix(), trajectory.Id, trajectory.Score) //输出文件路径
...
// 				var tmp [][]byte
// 				tmp = append(tmp, buf_cutted.GetBytes())
// 				imgMap[strconv.FormatInt(trajectory.Id, 10)] = tmp
// 				tmp = nil
// 			} else { //map[key]不为nil 添加
// 				tmp := imgMap[strconv.FormatInt(trajectory.Id, 10)]
// 				tmp = append(tmp, buf_cutted.GetBytes())
// 				imgMap[strconv.FormatInt(trajectory.Id, 10)] = tmp
// 			}
...
// 		//不断获取最新1s的最好图片
// 		if time.Now().After(t1.Add(time.Second)) {
// 			t1 = time.Now() //重新计算1s
// 			// time.
// 		}
// 	}
...
// 			fmt.Println(strconv.Itoa(len(value)))
// 			strTmp := strconv.Itoa(len(value)) + ","
// 			size += strTmp
// 			imgStr += string(value)
// 		}
