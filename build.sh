...
function build-and-push () {
    function fmt-info() { echo -e "\033[32m[info] $1\033[0m"; }
    function fmt-error() { echo -e "\033[31m[error] $1\033[0m"; }
    local project_name=$1
    local build_arch=$2
    local tag=$3
    local dist_image=aiedge/${project_name}:${tag}
    fmt-info "build for ${build_arch}"
    case ${build_arch} in
    "amd64")
        # dist_image=aiedge/${project_name}:${tag}
        fmt-info "building image ${dist_image}"
    ...
    "arm64")
        ...
        if [ -f "${build_arch}.dockerfile" ]; then
            ...
            fmt-info "no ${build_arch}.dockerfile, use dockerfile"
            docker build -t ${dist_image} .
        fi
        ;;
    *)
        fmt-error "$build_arch arch is not supported"
        exit 1
        ;;
    esac
    fmt-info "pushing image ${DOCKER_REGISTRY_SERVER}/${dist_image}"
